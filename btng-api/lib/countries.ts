 ...
 * Centralized country information for BTNG Trust Union network
 */
...
/**
 * Get all countries by status
 */
export function getCountriesByStatus(status: CountryData['status']): [string, CountryData][] {
  return Object.entries(countryData).filter(([_, country]) => country.status === status)
}

/**
 * Get country data by key
 */
export function getCountry(key: string): CountryData | undefined {
  return countryData[key]
}

/**
 * Get total active users across all countries
 */
export function getTotalActiveUsers(): number {
  return Object.values(countryData).reduce((total, country) => total + country.activeUsers, 0)
}

/**
 * Get total trust nodes across all countries
 */
export function getTotalTrustNodes(): number {
  return Object.values(countryData).reduce((total, country) => total + country.trustNodes, 0)
}

/**
 * Get all mobile money providers across the network
 */
export function getAllMobileMoneyProviders(): string[] {
  const providers = new Set<string>()
  Object.values(countryData).forEach(country => {
    country.mobileMoney.forEach(provider => providers.add(provider))
  })
