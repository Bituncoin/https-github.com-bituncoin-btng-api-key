...
import { fabricMint } from '@/lib/fabric/wallet-bridge';
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.error('Wallet mint error:', error);
        return NextResponse.json(
            { error: 'Mint bridge failure', status: 'failed' },
            { status: 503 }
        );
    }
}
