...
class Transaction:
    ...
    def sign(self, keypair, sighash_type=0x01):
        """
        Sign transaction (simplified P2PKH signing)
        ...
        # Real implementation would handle multiple inputs, different script types, etc.

        if len(self.inputs) != 1:
            raise NotImplementedError("Multi-input signing not implemented")

        # Create sighash
        sighash = self._sighash(0, sighash_type)
        ...
        self.inputs[0]['script_sig'] = script_sig

        return self.serialize()

    def _sighash(self, input_index, sighash_type):
        """Create sighash for signing"""
        # Simplified implementation
        return hash256(self.serialize() + int_to_bytes(sighash_type, 4, 'little'))
